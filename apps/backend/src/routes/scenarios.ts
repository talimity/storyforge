...
export async function scenariosRoutes(fastify: FastifyInstance) {
  ...
  // Get single scenario
  fastify.get<{ Params: GetScenarioParams }>('/api/scenarios/:id', async (request, reply) => {
    const { id } = request.params;
    console.log(`Fetching scenario with ID: ${id}`);
    console.log(`Available scenarios: ${mockScenarios.map(s => s.id).join(', ')}`);
  ...
  // Create scenario
  fastify.post<{ Body: Omit<Scenario, 'id'> }>('/api/scenarios', async (request, reply) => {
  ...
  // Update scenario
  fastify.put<{ Params: GetScenarioParams; Body: Partial<Scenario> }>('/api/scenarios/:id', async (request, reply) => {
    const { id } = request.params;
  ...
  // Delete scenario
  fastify.delete<{ Params: GetScenarioParams }>('/api/scenarios/:id', async (request, reply) => {
    const { id } = request.params;
    const scenarioIndex = mockScenarios.findIndex(s => s.id === id);
