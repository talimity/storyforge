...
import { useModelProfileSearch } from "@/lib/hooks/use-model-profile-search";

type MPItem = ReturnType<typeof useModelProfileSearch>["modelProfiles"][number];

function useModelProfileCollection(enabled: boolean) {
  const { modelProfiles, isLoading, updateSearch } = useModelProfileSearch({
    enabled,
  });
  const { collection, set } = useListCollection<MPItem>({
    initialItems: [],
